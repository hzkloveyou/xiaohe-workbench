...
import { isBookmarkEntity, type BookmarkEntity } from "./bookmark-model";
...
export function BookmarkGrid({ bookmarks, groups, onAdd, onEdit, onDelete, onReorder }: BookmarkGridProps) {
  ...
  const visible = useMemo(
    () => bookmarks.filter(isBookmarkEntity).filter((bookmark) => activeGroup === "all" || bookmark.data.groupId === activeGroup).sort((a, b) => a.data.order - b.data.order),
        ...
        <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
          <SortableContext items={visible.map((bookmark) => bookmark.id)} strategy={rectSortingStrategy}>
            <div className="bookmark-grid">
              {visible.map((bookmark) => <BookmarkCard key={bookmark.id} bookmark={bookmark} onEdit={onEdit} onDelete={onDelete} />)}
            </div>
          </SortableContext>
        </DndContext>
      )}
