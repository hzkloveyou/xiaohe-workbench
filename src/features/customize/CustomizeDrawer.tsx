  ...
  return (
    <Dialog open title="个性化工作台" onClose={props.onClose}>
      <div className="customize-content">
        ...
        <fieldset className="integration-preferences"><legend>搜索与连接</legend><label>默认搜索引擎<select value={props.preferences.searchEngine} onChange={(event) => props.onPreferencesChange({ searchEngine: event.target.value as WorkspacePreferences["searchEngine"] })}><option value="bing">Bing</option><option value="baidu">百度</option><option value="google">Google</option></select></label><label>GitHub 用户名<input value={props.preferences.githubUsername} onChange={(event) => props.onPreferencesChange({ githubUsername: event.target.value })} maxLength={39} placeholder="例如 hzkloveyou" /></label></fieldset>
